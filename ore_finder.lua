...
-- v3.0-final - By CedarI, cleanup by Gemini (rewritten/fixed)
---------------------------------------------------------------------
...
local function drawHeader(title)
    ...
    term.setCursorPos(1, 2); term.write(string.rep("-", W - 1))
end
...
local function drawMainMenu()
    ...
    local y = 6
    for i, c in ipairs(ORE_CATEGORIES) do
        term.setCursorPos(3, y)
        term.setTextColor(c.color)
        term.write(i .. ". " .. c.name)
        y = y + 1
    end
    term.setTextColor(colors.white)
    term.setCursorPos(1, H); term.write("Press number or 'q' to quit")
end
...
local function updateTrackingScreen(dist, dx, dy, dz)
    for y = 4, H - 2 do
        term.setCursorPos(1, y); term.write(string.rep(" ", W))
    end

    term.setCursorPos(1, 4); term.write("Tracking: " .. state.target_ore.name .. " (GPS)")
    ...
    if dist < 2.5 then
        ...
        term.setCursorPos(ax - 2, ay); term.write("[  HERE  ]")
    else
...
local function performScan(is_silent)
    if not is_silent then
        term.setCursorPos(1, 5); term.write("Scanning...")
    end
    state.last_scan_results = {}
    local all_blocks, _ = geoscanner.scan(SCAN_RADIUS)
    if all_blocks then
        local ore_lookup = {}
        for _, n in ipairs(state.selected_ore.blocks) do ore_lookup[n] = true end
        for _, block in ipairs(all_blocks) do
            if block and block.name and ore_lookup[block.name] then
                ...
                if USE_GPS and state.has_fix then
                    abs_x = state.player_pos.x + block.x
                    abs_y = state.player_pos.y + block.y
                    abs_z = state.player_pos.z + block.z
                end
                local dist = math.sqrt(block.x^2 + block.y^2 + block.z^2)
                table.insert(state.last_scan_results, {
                    dist = dist,
                    x = block.x,
                    y = block.y,
                    z = block.z,
...
local function mainLoop()
    while true do
        ...
        if state.current_menu == "main" then
            ...
            if c and ORE_CATEGORIES[c] then
                state.selected_category = ORE_CATEGORIES[c]
        ...
        elseif state.current_menu == "ore_select" then
            if key == "b" then
                ...
                local c = tonumber(key)
                if c and state.selected_category.ores[c] then
                    state.selected_ore = state.selected_category.ores[c]
        ...
        elseif state.current_menu == "scanning" then
            ...
            elseif key == "r" then
                performScan(false)
            elseif key == "t" and #state.last_scan_results > 0 and USE_GPS and state.has_fix then
                state.target_ore = {
                    name = state.selected_ore.name,
                    block_name = state.last_scan_results[1].name,
                    abs = state.last_scan_results[1].abs
                }
...
local function trackingLoop()
    ...
    local function update()
        updatePlayerPosition()
        if not (USE_GPS and state.has_fix and state.target_ore and state.target_ore.abs) then
            term.setCursorPos(1, 5); term.write("Waiting for GPS fix or target data...")
            return
        end
        local t = state.target_ore.abs
        local dx = t.x - state.player_pos.x
        local dy = t.y - state.player_pos.y
        local dz = t.z - state.player_pos.z
        local dist = math.sqrt(dx^2 + dy^2 + dz^2)
        updateTrackingScreen(dist, dx, dy, dz)
    end
